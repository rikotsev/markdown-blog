...
const DesignArticle: React.FC = () => {
        ...
        api
            ...
            .then((response) => {
                if (response.status == 201) {
                    navigate(`/category/${articleData.category!.id}/articles/${response.headers['location']}`)
                }
    ...
    if (id) {
        actionButton = (
            <button className={styles["create-article-btn"]} onClick={editArticle}>
                Save Changes
            </button>
        );
    } else {
        actionButton = (
            <button className={styles["create-article-btn"]} onClick={addArticle}>
                Create Article
            </button>
        );
    }
