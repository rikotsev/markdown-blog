...
const CategoryApiContext = createContext<CategoryApiData | undefined>(
  undefined,
);
...
export const CategoryApiProvider: React.FC<CategoryApiProviderProps> = ({
  children,
}) => {
  ...
  const add = useCallback(
    async (newCategory: CategoryCreate) => {
      try {
        console.log(api);
  ...
  return (
    <CategoryApiContext.Provider
      value={{ categories, api, withAuth, refreshCategories, add, remove }}
    >
      {children}
    </CategoryApiContext.Provider>
  );
